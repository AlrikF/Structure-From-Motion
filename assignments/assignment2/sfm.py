...
def set_arguments(parser): 
    ...
    #directory stuff
    parser.add_argument('--data_dir',action='store',type=str,default='./SfM_quality_evaluation/Benchmarking_Camera_Calibration_2008',dest='data_dir',
...
class SFM(object): 
    ...
    def load_matches(self, name1, name2): 
            ...
            In this name 1 number should be less than name 2 number (The pickle files are named in that way).
            Args:
    ...
    def baseline_pose_estimation(self, name1, name2):
            """
            Baseline pose estimation for Structure-from-Motion (SfM) typically involves estimating 
            the initial camera poses for a set of images
            ...
            ##!!! Important: The matches should be sorted in ascending order of distance before processing because the matches are small number , larger picture number 
            img1pts, img2pts, img1_kp_idx, img2_kp_idx = self.get_aligned_matches(kp1,desc1,kp2,desc2,matches)
            ...
            # Essential matrix K transpose*F*K
            E = self.K.T.dot(F.dot(self.K))

            # Used to find the rotation and translation between two cameras
            ...
            self.matches_data[(name1,name2)] = [matches, img1pts[mask], img2pts[mask], img1_kp_idx[mask],img2_kp_idx[mask]]
    ...
    def triangulate_two_views(self, name1, name2): 
        ...
        def triangulation(img1pts, img2pts, R1, t1, R2, t2): 
            ...
            """
            print('img1pts',img1pts)

            img1ptsHom = cv2.convertPointsToHomogeneous(img1pts)[:,0,:]
            img2ptsHom = cv2.convertPointsToHomogeneous(img2pts)[:,0,:]

            print('img1ptsHOM',img1ptsHom)
        ...
        # Image keypoints and keypoint indexes 
        ...
        for prev_name in self.image_data.keys(): 
            if prev_name != name: 
                ...
                if len(matches) > 0: 
                    ...
                    img1_kp_pts, img2_kp_pts, img1_idx, img2_idx = self.get_aligned_matches(kp1,desc1,kp2,desc2,matches)
                    
                    F,mask = cv2.findFundamentalMat(img1_kp_pts,img2_kp_pts,method=opts.fund_method,ransacReprojThreshold=opts.outlier_thres,confidence=opts.fund_prob)
                    
                    mask = np.reshape(np.bool_(mask), -1)
                    self.matches_data[(prev_name,name)] = [matches, img1_kp_pts[mask], img2_kp_pts[mask], img1_idx[mask],img2_idx[mask]]
    ...
    def compute_reprojection_error(self, name): 
        ...
        ############################################
        pos_ref = [int(val) for val in ref if val > 0]
        transpose_pt_cloud = self.point_cloud[pos_ref].T

        homogenous_pts = self.K.dot(R.dot(transpose_pt_cloud) + t )
        reprojected_pts = cv2.convertPointsFromHomogeneous(homogenous_pts.T)[:,0,:]

        img_pts = np.array([kp_.pt for i, kp_ in enumerate(kp) if ref[i] > 0])
        
        #Calculating the mean of the euclidean distance between the reprojected points and the image points
        err = np.mean(np.sqrt(np.sum((img_pts-reprojected_pts)**2,axis=-1)))
        ...
        if self.opts.plot_error: 
            ...
            ax = draw_correspondences(image, img_pts, reprojected_pts , ax)
