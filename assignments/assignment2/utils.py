...
def draw_correspondences(img, ptsTrue, ptsReproj, ax, drawOnly=50): 
    ...
    subset_points_indexes  = np.random.choice(ptsTrue.shape[0],size=(drawOnly,),replace=False)
    subset_ptsTrue   = ptsTrue[subset_points_indexes]
    subset_ptsReProj = ptsReproj[subset_points_indexes]
    ax.scatter(subset_ptsTrue[:,0]  ,subset_ptsTrue[:,1]  ,marker='o' ,c='green',linewidths=0.3, label='True points')
    ax.scatter(subset_ptsReProj[:,0],subset_ptsReProj[:,1],marker='x' ,c='red',linewidths=0.8, label='Reprojected points ')
    
    #Blue lines connecting true and reprojected points
    for i in range(drawOnly):
        ax.plot([subset_ptsTrue[i, 0], subset_ptsReProj[i, 0]], [subset_ptsTrue[i, 1], subset_ptsReProj[i, 1]], c='blue', linewidth=0.7, alpha=0.7)
